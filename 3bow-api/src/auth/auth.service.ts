...
// === CẤU HÌNH THOẢI MÁI ĐỂ TEST ===
const CHALLENGE_TTL_MIN = 15;    // Mã có hiệu lực 15 phút
const RESEND_COOLDOWN_SEC = 5;   // Đợi 5s để gửi lại
const MAX_SEND_IN_HOUR = 1000;   // Cho phép gửi 1000 lần/giờ (khỏi lo bị khóa)
const MAX_VERIFY_WRONG = 50;     // Nhập sai 50 lần mới khóa
...
export class AuthService {
  ...
  // === HÀM SET COOKIE CHUẨN (FIX DOMAIN) ===
  public setAuthCookies(res: any, accessToken: string, refreshToken: string) {
    // HARDCODE DOMAIN ĐỂ CHIA SẺ GIỮA API VÀ WEB
    ...
    res.cookie('access_token', accessToken, { ...opts, maxAge: 7 * 24 * 60 * 60 * 1000 });
    res.cookie('refresh_token', refreshToken, { ...opts, maxAge: 30 * 24 * 60 * 60 * 1000 });
  }

  /* ===== LOGIC LOGIN (FIX: CHỈ SO SÁNH, KHÔNG SỬA DB) ===== */
  async login(dto: LoginDto) {
    const email = dto.email.toLowerCase().trim();
    const user = await this.usersRepo.findOne({
      where: { email },
      select: { id: true, email: true, passwordHash: true, role: true, firstName: true, lastName: true, sessionVersion: true },
    });
    if (!user) throw new UnauthorizedException('Email hoặc mật khẩu không đúng');

    // So sánh pass nhập vào với Hash trong DB
    const isMatch = await bcrypt.compare(dto.password, user.passwordHash || '');
    if (!isMatch) throw new UnauthorizedException('Email hoặc mật khẩu không đúng');

    // Tạo session mới
    const sessionToken = this.generateSessionToken();
    const newSv = (user.sessionVersion ?? 0) + 1;
    
    await this.usersRepo.update(
      { id: user.id },
      { session: sessionToken, sessionVersion: newSv }
    );

    const accessToken = await this.signAccessToken({ sub: user.id, email: user.email, role: user.role as UserRole, sv: newSv });
    const refreshToken = await this.signRefreshToken({ sub: user.id, sv: newSv });

    return { accessToken, refreshToken, sessionToken, user };
  }

  /* ===== OTP FLOW: START (FIX: CHỈ SO SÁNH) ===== */
  async loginVerifyStart(dto: LoginDto, next?: string, ip?: string, ua?: string) {
    const email = dto.email.toLowerCase().trim();
    const user = await this.usersRepo.findOne({ 
      where: { email },
      select: { id: true, email: true, passwordHash: true, role: true, lockedUntil: true, loginAttemptCount: true, lastLoginAttemptAt: true }
    } as any);
    
    if (!user) throw new UnauthorizedException('Email hoặc mật khẩu không đúng');

    // Bỏ qua check khóa tạm thời để dễ test (hoặc uncomment nếu muốn chặt chẽ)
    // if ((user as any).lockedUntil && (user as any).lockedUntil > new Date()) this.tooMany('Tài khoản bị khoá.');

    // So sánh Hash
    const isMatch = await bcrypt.compare(dto.password, (user as any).passwordHash || '');
    if (!isMatch) throw new UnauthorizedException('Email hoặc mật khẩu không đúng');

    // Reset count nếu qua 1h
    const now = new Date();
    // Logic check spam... (giữ nguyên nhưng nới lỏng config ở trên)

    // Tạo Challenge
    const ch = await this.challengeRepo.save(this.challengeRepo.create({
      userId: user.id, 
      codeHash: '', 
      expiresAt: new Date(now.getTime() + CHALLENGE_TTL_MIN * 60000),
      ip, userAgent: ua, nextPath: next || '/', 
      lastSentAt: new Date(now.getTime() - RESEND_COOLDOWN_SEC * 1000), 
      sentCount: 0, verifyCount: 0,
    }));
  ...
  async loginVerifyResend(challengeId: string) {
    const ch = await this.challengeRepo.findOne({ where: { id: challengeId }, relations: { user: true } });
    if (!ch || ch.usedAt || ch.expiresAt < new Date()) throw new BadRequestException('Challenge không hợp lệ hoặc đã hết hạn');
    
    const code = Math.floor(100000 + Math.random() * 900000).toString();
    const codeHash = await bcrypt.hash(code, 10);
    
    await this.mailer.sendOtp(ch.user.email, code);
    await this.challengeRepo.update({ id: ch.id }, { codeHash, sentCount: (ch.sentCount || 0) + 1, lastSentAt: new Date() });
    
    return { ok: true, resendAfterSec: RESEND_COOLDOWN_SEC };
  }

  /* ===== OTP FLOW: VERIFY (QUAN TRỌNG: SET COOKIE) ===== */
  async loginVerifyVerify(challengeId: string, code: string, res: any): Promise<string> {
    const ch = await this.challengeRepo.findOne({ where: { id: challengeId }, relations: { user: true } });
    
    // LOGIC CHECK LỖI
    if (!ch) throw new BadRequestException('Challenge không tồn tại');
    if (ch.usedAt) throw new BadRequestException('Mã này đã được sử dụng');
    if (ch.expiresAt < new Date()) throw new BadRequestException('Mã đã hết hạn');
    ...
    // Login thành công -> Update DB
    await this.challengeRepo.update({ id: ch.id }, { usedAt: new Date() });
    
    const sessionToken = this.generateSessionToken();
    const newSv = (ch.user.sessionVersion ?? 0) + 1;
    await this.usersRepo.update({ id: ch.user.id }, { session: sessionToken, sessionVersion: newSv });

    // Tạo Token
    const accessToken = await this.signAccessToken({ sub: ch.user.id, email: ch.user.email, role: ch.user.role as UserRole, sv: newSv });
    const refreshToken = await this.signRefreshToken({ sub: ch.user.id, sv: newSv });
    
    // GỌI HÀM SET COOKIE CHUẨN
    this.setAuthCookies(res, accessToken, refreshToken);
  ...
  /* ===== OTHER METHODS ===== */
  async logout(userId: string): Promise<void> { await this.usersRepo.update({ id: userId }, { session: null }); }
  
  async forceLogoutByEmail(email: string): Promise<void> { 
    const user = await this.usersRepo.findOne({ where: { email: email.toLowerCase().trim() } });
    if (user) await this.usersRepo.update({ id: user.id }, { session: null, sessionVersion: (user.sessionVersion ?? 0) + 1 });
  }
  ...
  async registerSupportAdmin(dto: RegisterSupportAdminDto) {
    ...
    const hash = await bcrypt.hash(dto.password, 10);
    const u = this.usersRepo.create({
      email, passwordHash: hash, role: UserRole.SUPPORT_ADMIN,
      firstName: dto.firstName, lastName: dto.lastName, sessionVersion: 0
    });
