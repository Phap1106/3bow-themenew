...
export const apiPatch =<T = any>(u: string, d?: any, i?: Omit<ReqInit, "method" | "body">) =>
  request<T>(u, { ...i, method: "PATCH",body: d instanceof FormData ? d : d != null ? JSON.stringify(d) : undefined });
...
export const listArticles = async (p: { page?: number; limit?: number; q?: string } = {}) => {
  ...
  if (q) search.set("q", q);
  // add common fallbacks so BEs with different conventions still work
  search.set("_page", String(page));
  search.set("_limit", String(limit));
  search.set("pageSize", String(limit));
  search.set("skip", String(skip));
  search.set("take", String(limit));
  search.set("offset", String(skip));

  const res = await apiGet<any>(`/articles?${search.toString()}`);

  const items = (res?.items ?? res?.data ?? res?.rows ?? res?.results ?? []) as any[];
  const total = Number(
    res?.total ?? res?.count ?? res?.meta?.total ?? res?.meta?.count ?? 0
  );
  return { items, total } as { items: any[]; total: number };
};

export const createArticle = (data: any) => apiPost("/articles", data);
export const updateArticle = (id: string, data: any) => apiPatch(`/articles/${id}`, data);
export const deleteArticle = (id: string) => apiDelete(`/articles/${id}`);
export const getArticle   = (id: string) => apiGet(`/articles/${id}`);
export const getArticleBySlug = (slug: string) => apiGet(`/articles/${encodeURIComponent(slug)}`);
...
/* ====== Admin Notes (Ghi nhớ) ====== */
// Backend gợi ý endpoint: GET /admin-notes (trả về { id, content, updatedAt })
// và PUT /admin-notes { content }
export type AdminNote = { id: string; content: string; updatedAt?: string };
export const getAdminNote = () => apiGet<AdminNote | { content?: string }>(`/admin/notes`);
export const saveAdminNote = (content: string) => apiPut<AdminNote>(`/admin/notes`, { content });
