...
// ép render runtime, không prebuild
export const dynamic = "force-dynamic";
export const revalidate = 0;
...
// ❌ BỎ HẲN generateStaticParams để không fetch lúc build
// export async function generateStaticParams() { ... }
      ...
      <article className="container-max grid gap-8 px-4 py-8 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-zinc-600">
            ...
            <span className="inline-flex items-center gap-1"><Calendar className="w-4 h-4" /><time dateTime={a.publishedAt ?? undefined}>{formatDate(a.publishedAt)}</time></span>
            {"readingMinutes" in a && (a as any).readingMinutes ? (
              ...
              {more.map((post) => (
                <li key={post.slug} className="flex gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-xs text-zinc-500">{formatDate(post.publishedAt)}</div>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <div className="grid gap-6 mt-4 md:grid-cols-2 lg:grid-cols-4">
            {more.slice(0, 4).map((post) => (
              <Link key={post.slug} href={`/news/${post.slug}`} className="block overflow-hidden bg-white group rounded-2xl ring-1 ring-zinc-200">
                <div className="relative aspect-[16/10]">
                  <img src={post.image || FALLBACK_IMG} alt={post.title} loading="lazy" className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-[1.03]" />
                </div>
                <div className="p-4">
                  <h4 className="font-semibold line-clamp-2">{post.title}</h4>
                  {post.excerpt ? <p className="mt-1 text-sm line-clamp-2 text-zinc-600">{post.excerpt}</p> : null}
                </div>
              </Link>
            ))}
          </div>
