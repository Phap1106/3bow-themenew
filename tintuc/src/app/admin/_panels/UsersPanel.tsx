          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map((u) => {
                ...
                // Hiển thị session token (rút gọn)
                const sessionDisplay = u.session 
                  ? `${u.session.substring(0, 8)}...${u.session.substring(u.session.length - 8)}`
                  : "—";
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span 
                        ...
                        title={u.session || "Không có session"}
                      >
                        {sessionDisplay}
                      </span>
                    </td>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              await load(page, q);
                              toast.success("Đã đăng xuất user thành công!");
                            } catch (error: any) {
                              console.error("Error kicking support admin:", error);
                              toast.error(error?.message || "Đăng xuất user thất bại!");
                            }
                          }}
                        >
                          <X size={16} />
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              toast.error(error?.message || "Xóa user thất bại!");
                            }
                          }}
                        >
                          <Trash2 size={18} />
        ...
        <SupportForm
          ...
          onSubmit={async (d) => {
            try {
              ...
              setShowCreate(false);
              await load(1, q);
              toast.success("Tạo tài khoản support admin thành công!");
            } catch (error: any) {
              console.error("Error creating support admin:", error);
              toast.error(error?.message || "Tạo tài khoản support admin thất bại!");
            }
          }}
        />
      </Modal>
        ...
        {editing && (
          <EditSupportForm
            ...
            onSubmit={async (d: SupportEditInput) => {
              try {
                // Map SupportEditInput to SupportInput format
                const updateData = {
                  firstName: d.firstName,
                  lastName: d.lastName,
                  numberPhone: d.phone, // Map phone to numberPhone for API
                  address: d.address,
                  img: d.img,
                  password: d.password,
                };
                await updateSupport(editing.id, updateData);
                setEditing(null);
                await load(page, q);
                toast.success("Cập nhật thông tin support admin thành công!");
              } catch (error: any) {
                console.error("Error updating support admin:", error);
                toast.error(error?.message || "Cập nhật thông tin support admin thất bại!");
              }
