...
//   // required, nhưng cho phép UI set "" => undefined rồi validate "required"
...
// /* ===== Default values (UI muốn placeholder) ===== */
// const DEFAULTS: DefaultValues<FormValues> = {
//   name: "",
//   phone: "",
//   email: "",
//   url: "",
//   // 2 field dưới để undefined (UI select hiển thị placeholder),
//   // vẫn an toàn vì zod sẽ validate required khi submit
//   // -> cast sang FormValues cho RHF khỏi phàn nàn
...
//   // ép resolver vào đúng kiểu để tránh lỗi TS khác version
...
//   } = useForm<FormValues>({
//     resolver,
//     defaultValues: DEFAULTS as unknown as FormValues,
//   });
...
//       // payload khớp model Lead của Prisma/BE hiện có
//       const payload = {
//         name: data.name,
//         phone: data.phone,
...
//       reset(DEFAULTS as unknown as FormValues);
//     } catch (e: any) {
...
/* ================== Styles (theme amber) ================== */
const fieldWrap =
  "rounded-2xl border border-zinc-200 bg-white/70 p-2 transition-all duration-300 hover:border-zinc-300 hover:bg-white focus-within:border-amber-400 focus-within:shadow-[0_0_0_6px] focus-within:shadow-amber-400/10";
const inputBase =
  "w-full rounded-xl border-0 bg-transparent px-3 py-2.5 text-sm placeholder:text-zinc-400 focus:outline-none focus:ring-0";
...
export default function ContactFormRHF() {
  ...
  return (
    <form
      ...
      className="group relative overflow-hidden rounded-3xl border border-zinc-200 bg-white p-6 md:p-8 shadow-sm transition-all duration-500 hover:-translate-y-0.5 hover:shadow-xl"
    >
      {/* Aura gradient nhẹ (amber) */}
      <div className="absolute transition-opacity duration-700 opacity-0 pointer-events-none -inset-24 blur-3xl group-hover:opacity-100">
        <div className="absolute w-56 h-56 rounded-full -top-10 -left-10 bg-amber-300/20" />
        <div className="absolute w-56 h-56 rounded-full -bottom-10 -right-10 bg-orange-300/20" />
      </div>

      {/* Heading */}
      <h3 className="text-2xl font-extrabold tracking-wide uppercase text-zinc-900/90 md:text-3xl">
        Hãy để <span className="text-amber-700/90">3BOW</span> hiểu hơn về bạn
      </h3>
      <p className="mt-2 text-zinc-600">
        Điền thông tin để chúng tôi tư vấn giải pháp phù hợp nhất.
      </p>
      ...
      <div className="grid gap-4 mt-6 sm:grid-cols-2">
        ...
        <Field
          className={`sm:col-span-2 ${fieldWrap}`}
        ...
        <Field className={fieldWrap} error={errors.service?.message}>
          <div className="relative w-full">
            <select
              ...
              defaultValue=""
              aria-invalid={!!errors.service}
              className={`${inputBase} appearance-none pr-10 ${invalid(
                errors.service
              )}`}
            >
              <option value="" disabled>
              ...
              <option value={SERVICE.GOOGLE_ADS}>Google Ads</option>
              <option value={SERVICE.FACEBOOK_INSTAGRAM_ADS}>
                Facebook/Instagram Ads
              </option>
              <option value={SERVICE.TIKTOK_ADS}>TikTok Ads</option>
              <option value={SERVICE.SEO}>SEO</option>
              <option value={SERVICE.LANDING_TRACKING}>
        ...
        {serviceValue === SERVICE.OTHER && (
          <Field
            className={`sm:col-span-2 ${fieldWrap}`}
            error={errors.serviceText?.message}
          >
            <input
              {...register("serviceText")}
              placeholder="Mô tả dịch vụ bạn cần (ví dụ: Landing Page/Tracking/Khác...)"
        ...
        >
          <textarea
            ...
            placeholder="Bạn cần trợ giúp điều gì?"
            aria-invalid={!!errors.note}
            className={`${inputBase} min-h-[120px] py-3 ${invalid(
              errors.note
            )}`}
          />
        </Field>
      ...
      <div className="mt-4 space-y-3">
        <label className="flex items-start gap-3 text-sm text-zinc-700">
          <input
            type="checkbox"
            {...register("consent")}
            className="w-4 h-4 mt-1 rounded border-zinc-300 text-amber-600 focus:ring-amber-500"
          />
          <span>
            Tôi đã đọc và đồng ý với{" "}
            <a
        ...
        <div className="flex flex-col items-stretch gap-3 mt-1 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-zinc-500">
            Cam kết bảo mật thông tin • Phản hồi trong 24h (T2–T6).
          </p>
          ...
          <button
            ...
            ${
              isSubmitSuccessful
                ...
                : "bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:-translate-y-0.5 hover:shadow-md"
            }`}
          >
            {!isSubmitSuccessful && (
              <span className="pointer-events-none absolute inset-0 before:absolute before:inset-0 before:-translate-x-full before:bg-gradient-to-r before:from-transparent before:via-white/40 before:to-transparent before:skew-x-12 before:content-[''] before:transition-transform before:duration-700 group-hover:before:translate-x-full" />
